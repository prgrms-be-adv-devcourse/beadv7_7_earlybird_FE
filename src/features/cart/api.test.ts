...
describe("cart api", () => {
  it("fetchCart는 CART_SERVICE.cart를 GET한다", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith(CART_SERVICE.cart);
  ...
  it("addCartItems는 project cart 엔드포인트에 {items}를 POST한다", async () => {
    ...
    const result = await addCartItems(payload);
    expect(apiClient.post).toHaveBeenCalledWith(CART_SERVICE.items, payload);
    expect(result).toEqual(cart);
  });

  it("removeCartItem은 CART_SERVICE.item(rewardId)를 DELETE한다", async () => {
    const cart = {
      cartId: 1,
      userId: 1,
      itemCount: 0,
    ...
    expect(apiClient.delete).toHaveBeenCalledWith(CART_SERVICE.item(10));
    expect(result).toEqual(cart);
  });

  it("clearCart는 CART_SERVICE.cart를 DELETE한다", async () => {
    (apiClient.delete as any).mockResolvedValue({ data: { success: true, data: null, error: null } });
    await clearCart();
    expect(apiClient.delete).toHaveBeenCalledWith(CART_SERVICE.cart);
  });
});
