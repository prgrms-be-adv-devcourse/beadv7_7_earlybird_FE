...
- **`CHATBOT_HANDOFF.md`** — 챗봇("오목눈이") 기능 연동 작업 시작 전 필독. 위젯(`src/features/chat/`)은 이미 배치 응답 기준으로 구현돼 있고, BE가 SSE 스트리밍으로 전환됨(2026-08-24) — 이 문서에 새 SSE 계약(실제 캡처 예시 포함)과 `api.ts`/`hooks.ts`/`store.ts`에서 구체적으로 뭘 바꿔야 하는지 정리돼 있음. **챗봇 관련 작업을 시작하면 이 파일부터 읽을 것.**
