...
- **프로젝트 썸네일 카드**: `splitIntoProjectSegments`(줄 단위 `-{3,}` 매칭) + `ProjectThumbnail`(`ChatWidget.tsx`)로 문서에 적힌 위치 매칭·`projects[0]` 즉시 렌더·세그먼트 불일치 시 하단 폴백까지 그대로 구현.
...
event:chunk
data:---
... (가죽 노트커버 소개 텍스트, 마지막 항목이라 뒤에 --- 없음) ...
```
(위 두 예시 다 SSE 데이터가 실제로는 한 글자/토큰 단위 `chunk`로 잘게 쪼개져 온다 — 여기선 가독성을 위해 문장 단위로 요약함. 실제 원시 스트림은 예시 A처럼 `data:` 한 줄에 글자 하나~몇 글자씩 옴.)
...
## 프로젝트 썸네일 카드 — `metadata.projects[]` + `---` 텍스트 구분선 (2026-08-24 추가)

여러 프로젝트를 추천하는 답변에 작은 썸네일+제목 헤더를 붙이기 위한 설계. BE는 이미지를 답변 텍스트 안에 마크다운(`![]()`)으로 섞어 보내지 않는다 — 지금 `renderMarkdownContent`가 자체 제작 경량 파서라 이미지 문법이 없고, SSE가 글자 단위로 청크를 쪼개 보내는 구조상 마크다운 문법 자체가 청크 경계에서 잘릴 위험이 있어서 의도적으로 피한 것. 대신 **구조화 데이터(`metadata.projects[]`) + 텍스트의 가벼운 구분선(`---`)** 조합으로 감.

**페이로드**: `metadata.projects: { projectId: number, title: string, thumbnailUrl: string | null }[]`
- **배열은 항상 그 답변에서 추천된 프로젝트 전체 개수·순서 그대로 나옴 — 썸네일이 없는 프로젝트도 배열에서 빠지지 않고 `thumbnailUrl: null`로 자리를 지킴.** 즉 FE도 이 배열을 필터링하면 안 됨(아래 위치 매칭이 깨짐).
- `thumbnailUrl`이 null이면 이미지 자리에 플레이스홀더 아이콘을 넣어 카드 높이를 통일하는 걸 권장(선택 사항).

**텍스트와의 결합 방식 — `---` 구분선 + 위치 인덱스 매칭**:
1. BE 시스템 프롬프트가 "여러 프로젝트 추천 시 프로젝트 사이마다 그 줄에 정확히 `---`(하이픈 3개 이상)만 있는 구분선을 넣어라, 프로젝트가 하나뿐이면 넣지 마라"고 LLM에 지시해둠 — 즉 N개 프로젝트를 추천하면 텍스트에 `---`가 N-1번 나옴(항목 사이 구분자라 마지막 항목 뒤엔 안 붙음). 실측 확인됨(4개 추천 시 `---` 정확히 3번).
2. FE는 누적된 텍스트를 trim 후 `-{3,}`에 매치되는 줄 기준으로 세그먼트로 나눔 — **단, `metadata.projects`가 비어있지 않을 때만 이 파싱을 켤 것.** 정책 질문 등 무관한 답변에서 `---`가 우연히 나올 수 있어, 프로젝트 추천이 아닐 때 이걸 구분선으로 오인하면 안 됨.
3. **핵심 규칙 — 세그먼트 0(첫 프로젝트)은 `---`를 기다리지 않는다.** `metadata` 이벤트는 항상 `chunk`(텍스트)보다 먼저 도착하므로, FE는 텍스트가 한 글자도 오기 전에 이미 `projects[0]`이 뭔지 알고 있음 — 그래서 `projects[0]`의 헤더(썸네일+제목)는 **`metadata` 도착 즉시** 렌더하면 됨. `---`는 "2번째 프로젝트부터"의 구분자일 뿐이라, 프로젝트가 1개뿐인 답변(구분선이 아예 없음)에도 이 규칙이 그대로 적용됨 — `projects[0]` 하나만 있어도 `metadata` 도착 즉시 헤더가 뜬다.
4. 이후 `---`를 만날 때마다(2번째, 3번째...) 그 순간 `projects[1]`, `projects[2]`...의 헤더를 얹고, 그 뒤로 이어지는 텍스트(LLM이 쓰는 프로젝트 설명+리뷰 요약 프로즈)는 평범하게 지금처럼 타이프라이터로 흘러가면 됨 — "블록이 끝나길 기다렸다 이미지 삽입"이 아니라 "블록 시작 시점에 헤더부터 보여주기".
5. **세그먼트 개수와 `projects.length`가 안 맞으면**(LLM이 구분선을 빠뜨리거나 더 쓴 경우 — 드물지만 100% 보장은 안 됨) 위치 매칭을 포기하고, 텍스트는 그냥 그대로 두고 `projects[]`를 답변 하단에 카드 블록으로 나열하는 방식으로 그레이스풀 폴백할 것.
