...
**[2026-08-26 추가, FE 반영 완료]** BE가 `조우진/chat-service/chat-issue-fixes`(이슈 #732) 브랜치에서 4가지를 고쳤는데, 이 중 `metadata` 이벤트 관련 버그 수정 1건만 FE 계약에 영향이 있음(나머지 3건 — SSE 연결 종료 시 취소, 추천 후보 과다기록 수정, 정책 재색인 자동화 — 은 순수 BE 내부 변경이라 FE 영향 없음, 참고만). 영향 있는 1건: **`metadata`가 이제 한 턴에 여러 번 올 수 있음**(예전엔 "정확히 1번"이었음). BE 코드에 이어 FE 코드도 같은 날 반영 완료 — 상세는 아래 "`metadata`가 한 턴에 여러 번 올 수 있음" 섹션의 "FE 구현 완료" 하위 섹션.
...
- **`event: tool_start`가 tool을 실제로 부를 때마다(한 턴에 여러 번 가능) 옴** — 상세는 아래 "새 SSE 이벤트: `tool_start`" 섹션. **[2026-08-26 갱신] 예전엔 "항상 `metadata`보다 먼저"라고 적혀 있었는데 더 이상 보장 아님** — 바로 아래 항목 및 "`metadata`가 한 턴에 여러 번 올 수 있음" 섹션 참고.
- **`event: metadata`** — `{ toolsUsed: string[], references: {category, topic}[], projects: {projectId, title, thumbnailUrl}[] }`. **[2026-08-26 갱신] "항상 정확히 1번, 텍스트보다 먼저 옴"이라던 예전 설명은 더 이상 정확하지 않음** — 이제 한 턴에 여러 번 올 수 있고, 매번 그 시점까지의 전체 스냅샷을 다시 보낸다. 상세는 아래 "`metadata`가 한 턴에 여러 번 올 수 있음" 섹션. `projects`는 상세는 아래 "프로젝트 썸네일 카드" 섹션.
...
2. `event: metadata`가 도착하면, **그 시점까지 스택에 쌓인 항목을 `completedMessage`(완료형)로 전환**. 개별 tool이 끝날 때마다 따로 신호를 안 주는 이유: `metadata`는 하나라도 tool이 실패하면 스트림이 에러로 끊겨 아예 안 오므로, "`metadata` 도착 = 그때까지 쌓인 tool_start가 다 성공적으로 끝났다"는 신뢰 가능한 신호로 그대로 쓸 수 있음 — 그래서 BE는 tool마다 별도 "완료" 이벤트를 안 보내도 됨. **[2026-08-26 갱신, FE 반영 완료] `metadata`가 이제 한 턴에 여러 번 올 수 있고 `tool_start`보다 먼저 올 수도 있음(아래 섹션 참고)** — 이 때문에 완료 여부를 **메시지 전체의 boolean 하나**로 두면 안 되고 **항목별로** 둬야 함: `metadata` 도착 시점에 스택에 이미 있던 항목만 완료형으로 바뀌고, 그 이후 새로 도착하는 `tool_start`는 다시 진행형으로 시작해 다음 `metadata`를 기다린다(그래야 "방금 막 시작한 tool이 곧바로 완료로 보이는" 오표시가 안 생김 — 초기 구현에서 실제로 이 버그가 있었고 항목별 완료 상태로 재설계해서 고침, `types.ts`의 `ToolProgressEntry.completed` 참고). `onMetadata` 콜백을 "1번만 처리하고 이후 호출은 무시"하도록 구현했다면 그 가드도 제거해야 함 — 두 번째 이후 호출도 매번 유효한 갱신이다.
3. 이 스택은 최종 답변 텍스트가 다 온 뒤(또는 다음 사용자 메시지를 보낼 때) 지워지는 게 자연스러움 — 계속 쌓이게 두면 지저분해짐.

## `metadata`가 한 턴에 여러 번 올 수 있음 (2026-08-26, BE 버그 수정, FE 반영 완료)

**BE 이슈**: [#732](https://github.com/prgrms-be-adv-devcourse/beadv7_7_earlybird_BE/issues/732), 브랜치 `조우진/chat-service/chat-issue-fixes`. 지금까지 이 문서는 "`event: metadata`는 정확히 1번, tool 호출이 전부 끝난 뒤에만 온다"고 전제했다(BE `ChatOrchestrationService`가 "구독자에게 도착하는 첫 content chunk = tool 호출이 전부 끝난 신호"라고 가정했기 때문). Spring AI 소스를 직접 확인해보니 이 전제가 항상 성립하지는 않는다는 게 드러났다 — **모델이 tool을 호출하기 전에 안내 텍스트를 먼저 출력하는 경우, 그 텍스트 청크가 tool 실행이 끝나기도 전에 먼저 도착할 수 있다.** 이 경우 예전 BE 코드는 `metadata`를 빈 상태(`toolsUsed`/`references`/`projects` 전부 `[]`)로 딱 한 번 보내고 그걸로 끝이라, 실제 tool 결과(프로젝트 카드, 정책 출처 등)가 최종 응답에서 통째로 누락될 수 있었다.

**BE가 고친 방식**: "한 번 보내고 잠금" 대신, **tool 실행 상태가 바뀔 때마다(tool이 시작될 때, 프로젝트/정책 출처가 새로 기록될 때) `metadata`를 다시 보낸다.** 매번 보내는 페이로드는 델타(증분)가 아니라 **그 시점까지의 전체 스냅샷**이고, 이전 이벤트가 담고 있던 내용을 그대로 포함하는 상위집합이다(recorder가 누적만 하지 지우지 않으므로) — 즉 **가장 최근에 받은 `metadata`가 항상 최신이자 가장 정확한 상태**다.

**FE가 반영해야 할 것**:
1. `onMetadata`(또는 이에 대응하는 콜백/이벤트 핸들러)가 **한 턴에 여러 번 호출될 수 있다는 전제로 구현**할 것 — "이미 한 번 처리했으면 이후 호출 무시" 같은 1회성 가드가 있다면 제거.
2. `projects[]`를 `title → ProjectCard` 맵으로 저장해두는 로직(아래 "프로젝트 썸네일 카드" 섹션)은 **매번 새로 받은 배열로 맵을 다시 만들면(덮어쓰기) 안전함** — 이전 맵에 있던 항목이 사라지는 일은 없다(상위집합 보장). 별도의 병합(merge) 로직을 짤 필요는 없고, 단순 교체로 충분하다.
3. `toolsUsed`/`references`도 같은 이유로 단순 교체로 충분 — 다만 이 두 값을 화면에 실시간으로 쓰는 UI가 있다면(현재는 없는 것으로 보임, `toolsUsed`는 디버깅용으로만 쓰고 있었음) 값이 중간에 갱신될 수 있다는 점 감안.
4. 이벤트 순서 보장 완화: 위에서 언급했듯 이제 드물게 `metadata`가 `tool_start`보다 먼저 오는 순서(안내 텍스트 leak 케이스)가 있을 수 있다 — `tool_start` → `metadata` 순서를 하드하게 가정하는 코드가 있다면(예: "`metadata` 받기 전엔 `tool_start`가 최소 1번은 있었다"고 가정) 깨질 수 있으니 확인 필요.

**BE도 완전히 못 없앤 것**: 이 순서 역전 자체(드물게 `metadata`가 `tool_start`보다 먼저 오는 것)는 Spring AI 프레임워크 내부 구조 때문에 BE 코드만으로 완전히 막을 수 없다 — BE가 보장하는 건 "내용은 항상 최종적으로 정확하다"까지고, "순서"까지는 아니다. 실서버 재현 검증(모델이 실제로 이런 응답을 내는 케이스)은 BE 쪽에서도 아직 못 했다 — 재현 자체가 드물어서 관건.

**FE 구현 완료 (2026-08-26)**: 위 "FE가 반영해야 할 것" 4개 항목을 `src/features/chat/`에 전부 반영함.

1. **1회성 가드**: 애초에 없었음(`hooks.ts`의 `onMetadata`는 처음부터 매 호출마다 그대로 실행되는 구조) — 확인만 하고 코드 변경 없음.
2. **`projects[]` 덮어쓰기**: `hooks.ts`의 `onMetadata`에 `if (meta.projects && meta.projects.length > 0)` 조건이 있어서 빈 배열이 오면 `setLastMessageProjects`를 호출하지 않는 구조였음(상위집합 보장 하에서는 결과가 같지만 "매번 무조건 덮어쓰기"는 아니었음) — 조건 제거, 이제 매 `metadata`마다 무조건 `setLastMessageProjects(meta.projects)` 호출.
3. **`references`/`toolsUsed`**: `references`는 이미 무조건 덮어쓰기였음(변경 없음). `toolsUsed`는 애초에 어디에도 저장/렌더하지 않고 파싱만 하고 버리는 구조라(디버깅용 필드로만 타입에 존재) 고칠 대상 자체가 없음.
4. **순서 가정 완화 — 실제 버그를 찾아 고침**: `store.ts`의 `completeToolProgress()`가 그 턴의 `toolProgress` 배열 전체에 **메시지 레벨 boolean 하나**(`toolProgressCompleted`)를 세팅하고, `ChatWidget.tsx`의 `ToolProgressStack`이 이 하나의 boolean으로 스택 안 **모든 항목**을 완료형(`completedMessage`)으로 렌더하는 구조였다. `metadata`가 `tool_start`보다 먼저 오면(위에서 설명한 순서 역전 케이스), 그 이후에 도착하는 새 `tool_start`가 이미 `true`로 세팅된 완료 플래그를 그대로 물려받아 **막 시작한 tool이 시작하자마자 체크마크 + 완료 문구로 잘못 표시되는 버그**가 있었다 — 실사용자가 "챗봇이 '잠시만 기다려주세요' 하고 멈춘 것처럼 보인다"고 보고한 증상의 유력한 원인. **수정**: 완료 여부를 메시지 전체가 아니라 `toolProgress` 항목 하나하나에 붙이도록 재설계함.
   - `types.ts`: `ToolProgressEntry extends ToolStartEvent { completed: boolean }` 신설, `ChatMessage.toolProgress?: ToolProgressEntry[]`로 교체(기존 `toolProgressCompleted?: boolean` 필드는 삭제).
   - `store.ts`: `addToolProgress`가 이제 `{ ...event, completed: false }`를 스택에 추가. `completeToolProgress`는 **그 시점까지 스택에 이미 쌓여있는 항목만** `completed: true`로 바꾸고, 그 이후 새로 도착하는 `tool_start`는 다시 `completed: false`로 새로 시작 — 다음 `metadata`가 그 항목까지 따라잡을 때 완료 처리됨.
   - `ChatWidget.tsx`: `ToolProgressStack`이 메시지 레벨 `completed` prop 대신 `item.completed`(항목별)로 체크마크/문구를 렌더.
   - 브라우저에서 실제로 여러 tool을 순차 호출하는 질문(프로젝트 검색 → 리뷰 검색을 프로젝트마다 반복, tool_start 6회)으로 검증 — 진행형→완료형 전환이 항목별로 올바르게 표시됨. `npx tsc --noEmit` 클린.
...
**FE 매칭 알고리즘**:
1. `metadata` 도착 시 `projects[]`를 `title → ProjectCard` 맵으로만 저장해둔다. **이 시점엔 아무 카드도 렌더하지 않음** — 예전 설계의 "`projects[0]` 즉시 렌더" 특례는 제거됨. 카드는 오직 텍스트에서 그 프로젝트의 헤더 줄이 실제로 등장하는 시점에만 나타난다. **[2026-08-26] `metadata`가 한 턴에 여러 번 올 수 있음(위 "`metadata`가 한 턴에 여러 번 올 수 있음" 섹션 참고) — 매번 이 맵을 새로 받은 배열로 덮어쓰면 되고, 별도 병합 로직은 필요 없다.**
